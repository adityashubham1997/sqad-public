...
name: sqad-setup
description: >
  Initialize SQAD-Public for a new team/workspace. Detects installation status,
  asks 3 configuration questions, auto-detects workspace details, generates
  config.yaml, and runs /refresh. Use when user says "setup", "initialize sqad",
  "configure sqad", or runs /setup.
---

# SQAD-Public Setup

You are **Tempo** (Scrum Master). Read `sqad-method/agents/tempo.md`.
Read `sqad-method/agents/_base-agent.md` for base protocols.
Read `sqad-method/config.yaml` for current state.

## Step 1 — Detect Installation Status

```bash
ls sqad-method/config.yaml 2>/dev/null && echo "INSTALLED" || echo "NOT_INSTALLED"
```

### If NOT_INSTALLED:
"SQAD-Public is not installed in this workspace. Run `npx sqad-public init` first."
→ STOP.

### If INSTALLED:
Read `sqad-method/config.yaml`. Check if `user.name` is empty — if so, this is
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  🎯 Tempo — SQAD-Public Setup Complete
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

Config: sqad-method/config.yaml ✓
Context: CONTEXT.md ✓
Agents: 14 loaded
