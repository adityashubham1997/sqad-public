...
const FRAMEWORK_DETECTORS = {
  javascript: [
    ...
    { dep: 'knex', framework: 'knex' },
    { dep: 'drizzle-orm', framework: 'drizzle' },
    { dep: 'mongoose', framework: 'mongoose' },
    { dep: 'pg', framework: 'postgres' },
    { dep: 'mysql2', framework: 'mysql' },
    { dep: 'better-sqlite3', framework: 'sqlite' },
    { dep: 'redis', framework: 'redis' },
    { dep: 'ioredis', framework: 'redis' },
    { dep: 'elasticsearch', framework: 'elasticsearch' },
    { dep: '@elastic/elasticsearch', framework: 'elasticsearch' },
  ],
  python: [
    ...
    { dep: 'pandas', framework: 'pandas' },
    { dep: 'numpy', framework: 'numpy' },
    { dep: 'scikit-learn', framework: 'sklearn' },
    { dep: 'sklearn', framework: 'sklearn' },
    { dep: 'torch', framework: 'pytorch' },
    { dep: 'pytorch', framework: 'pytorch' },
    { dep: 'tensorflow', framework: 'tensorflow' },
    { dep: 'keras', framework: 'keras' },
    { dep: 'xgboost', framework: 'xgboost' },
    { dep: 'lightgbm', framework: 'lightgbm' },
    { dep: 'mlflow', framework: 'mlflow' },
    { dep: 'wandb', framework: 'wandb' },
    { dep: 'dbt-core', framework: 'dbt' },
    { dep: 'great_expectations', framework: 'great-expectations' },
    { dep: 'pyspark', framework: 'spark' },
    { dep: 'polars', framework: 'polars' },
    { dep: 'airflow', framework: 'airflow' },
    { dep: 'prefect', framework: 'prefect' },
    { dep: 'dagster', framework: 'dagster' },
    { dep: 'streamlit', framework: 'streamlit' },
    { dep: 'sqlalchemy', framework: 'sqlalchemy' },
    { dep: 'SQLAlchemy', framework: 'sqlalchemy' },
    { dep: 'psycopg2', framework: 'postgres' },
    { dep: 'pymongo', framework: 'mongodb' },
    { dep: 'redis', framework: 'redis' },
    { dep: 'elasticsearch', framework: 'elasticsearch' },
  ],
  ruby: [
...
export function detectStack(workspacePath) {
  ...
  if (cppFiles.length > 0) {
    ...
    // Only default to 'c' if we found .h files but no .c/.cpp — .h alone is ambiguous
    // but convention is to treat standalone headers as C unless proven otherwise
    if (!hasC && !hasCpp) {
      if (cppFiles.some(f => f.endsWith('.h')) && !result.languages.includes('c')) {
  ...
  // Game engines
  detectGameEngines(workspacePath, result);

  // Schema & infra artifact detection (1.2.6)
  detectSchemaAndInfra(workspacePath, result);

  return result;
}

/**
 * Detect game engines (Unity, Unreal, Godot, MonoGame, SDL, Raylib).
 */
function detectGameEngines(workspacePath, result) {
  // Unity — Assets/ + ProjectSettings/ or *.unity files
  if (existsSync(join(workspacePath, 'Assets')) && existsSync(join(workspacePath, 'ProjectSettings'))) {
    if (!result.frameworks.includes('unity')) result.frameworks.push('unity');
    if (!result.languages.includes('csharp')) result.languages.push('csharp');
  }

  // Unreal Engine — *.uproject file
  const uprojectFiles = deepFindByExtension(workspacePath, ['.uproject'], 2);
  if (uprojectFiles.length > 0) {
    if (!result.frameworks.includes('unreal')) result.frameworks.push('unreal');
    if (!result.languages.includes('cpp')) result.languages.push('cpp');
  }

  // Godot — project.godot file
  if (existsSync(join(workspacePath, 'project.godot'))) {
    if (!result.frameworks.includes('godot')) result.frameworks.push('godot');
  }

  // MonoGame — MonoGame NuGet or Content Pipeline
  const csprojFiles = deepFindByExtension(workspacePath, ['.csproj'], 3);
  for (const csproj of csprojFiles) {
    try {
      const content = readFileSync(csproj, 'utf8');
      if (content.includes('MonoGame') || content.includes('monogame')) {
        if (!result.frameworks.includes('monogame')) result.frameworks.push('monogame');
        break;
      }
    } catch (e) { /* ignore */ }
  }
...
// Re-export deepFindByExtension for backward compatibility
export { deepFindByExtension } from './utils.js';

/**
 * Detect schema and infrastructure artifacts (1.2.6).
 * Adds: protobuf, graphql, openapi, migrations, terraform, helm.
 */
function detectSchemaAndInfra(workspacePath, result) {
  const add = (list, item) => { if (!list.includes(item)) list.push(item); };

  // Protobuf — *.proto files
  const protoFiles = deepFindByExtension(workspacePath, ['.proto'], 4);
  if (protoFiles.length > 0) add(result.frameworks, 'protobuf');

  // GraphQL — *.graphql or *.gql files
  const gqlFiles = deepFindByExtension(workspacePath, ['.graphql', '.gql'], 4);
  if (gqlFiles.length > 0) add(result.frameworks, 'graphql');

  // OpenAPI / Swagger — openapi.yaml/json, swagger.yaml/json
  const openApiNames = ['openapi.yaml', 'openapi.json', 'swagger.yaml', 'swagger.json'];
  if (deepFindByName(workspacePath, openApiNames, 4).length > 0) {
    add(result.frameworks, 'openapi');
  }

  // Database migrations — migrations/ or db/migrate/
  const migrationDirs = deepFindByName(workspacePath, ['migrations', 'migrate'], 3);
  if (migrationDirs.length > 0) add(result.frameworks, 'db-migrations');

  // Terraform — *.tf files
  const tfFiles = deepFindByExtension(workspacePath, ['.tf', '.hcl'], 4);
  if (tfFiles.length > 0) add(result.frameworks, 'terraform');

  // Helm — Chart.yaml
  const helmFiles = deepFindByName(workspacePath, ['Chart.yaml'], 4);
  if (helmFiles.length > 0) add(result.frameworks, 'helm');
}
