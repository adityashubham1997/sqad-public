...
export async function update(workspacePath, options = {}) {
  ...
  // Update framework dirs — preserve user data (config.yaml, output/, custom-agents/, plugins/)
  const methodSrc = join(PACKAGE_ROOT, 'squad-method');
  const updateDirs = ['agents', 'fragments', 'skills', 'tools', 'templates', 'context'];
  ...
  console.log('   Updated: agents/, fragments/, skills/, tools/, templates/, context/\n');
}
