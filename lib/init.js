...
export async function init(workspacePath, options = {}) {
  ...
  if (!existsSync(methodDest)) {
    ...
    // Sync framework content (agents, skills, tools, fragments) — preserve config.yaml and output/
    console.log('📁 squad-method/ exists — syncing agents, skills, tools, fragments...');
    const configPath = join(methodDest, 'config.yaml');
    const configBackup = existsSync(configPath) ? readFileSync(configPath, 'utf8') : null;
    for (const dir of ['agents', 'skills', 'tools', 'fragments', 'templates', 'context']) {
      const src = join(methodSrc, dir);
      const dest = join(methodDest, dir);
      if (existsSync(src)) cpSync(src, dest, { recursive: true });
    }
    if (configBackup) writeFileSync(configPath, configBackup, 'utf8');
  }
  ...
  } else if (ides.length === 0 && process.stdin.isTTY) {
    // Interactive picker when no IDEs detected and running in a terminal
    ides = await promptIdeSelection(ALL_IDES);
  } else if (ides.length === 0) {
    // Non-interactive fallback (piped stdin, CI, etc.) — deploy to all
    console.log('   No IDEs detected — deploying to all 7 IDEs by default.');
    ides = ALL_IDES;
  }
  console.log(`   IDEs: ${ides.map(i => i.name).join(', ') || 'none'}`);
 ...
 * Shows a checklist of all 7 IDEs and waits for user input.
 */
async function promptIdeSelection(allIdes) {
  const rl = createInterface({ input: process.stdin, output: process.stdout });
  const question = (q) => new Promise(resolve => rl.question(q, resolve));

  console.log('\n   Which IDEs would you like to configure?');
  allIdes.forEach((ide, i) => console.log(`   ${i + 1}. ${ide.name} (${ide.id})`));
  console.log('   (Enter numbers, comma-separated, or "all" for all IDEs)');

  const answer = await question('   → ');
  rl.close();

  const trimmed = answer.trim().toLowerCase();
  if (!trimmed || trimmed === 'all') return allIdes;

  const selected = [];
  for (const part of trimmed.split(',')) {
    const n = parseInt(part.trim(), 10);
    if (!isNaN(n) && n >= 1 && n <= allIdes.length) {
      selected.push(allIdes[n - 1]);
    } else {
      const byId = allIdes.find(i => i.id === part.trim());
      if (byId) selected.push(byId);
    }
  }
  return selected.length > 0 ? selected : allIdes;
}
