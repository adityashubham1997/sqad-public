...
### 2c. Key Artifacts — Full Workspace Scan

Scan ALL file categories (cap: top 20 per category, skip secrets and build artifacts):
...
# --- Extended categories (1.2 expansion) ---

# docs/ — Architecture docs, ADRs, README
find . -path ./node_modules -prune -o \( -name "*.md" -o -name "*.rst" -o -name "*.adoc" \) -print 2>/dev/null | grep -v ".git/" | head -20

# schemas/ — Data model definitions
find . -path ./node_modules -prune -o \( -name "*.proto" -o -name "*.graphql" -o -name "*.sql" \) -print 2>/dev/null | grep -v ".git/" | head -20
find . -path ./node_modules -prune -o -type d \( -name "migrations" -o -name "db" \) -print 2>/dev/null | head -5

# infra/ — Container and IaC
find . -path ./node_modules -prune -o \( -name "Dockerfile*" -o -name "docker-compose*" \) -print 2>/dev/null | grep -v ".git/" | head -10
find . -path ./node_modules -prune -o \( -name "*.tf" -o -name "*.hcl" \) -print 2>/dev/null | grep -v ".git/" | head -10
find . -path ./node_modules -prune -o \( -name "Chart.yaml" -o -type d -name "k8s" -o -type d -name "helm" \) -print 2>/dev/null | head -5
...
find . -path ./node_modules -prune -o \( -name "openapi.yaml" -o -name "openapi.json" -o -name "swagger.json" -o -name "swagger.yaml" \) -print 2>/dev/null | grep -v ".git/" | head -5

# config/ — Environment and build config
ls .env.example tsconfig.json Makefile 2>/dev/null
find . -path ./node_modules -prune -o \( -name "*.config.js" -o -name "*.config.ts" \) -print 2>/dev/null | grep -v ".git/" | head -10
```

**EXCLUSIONS — never include in context files:**
...
### CONTEXT.md line limit: 400 lines (root), 200 lines (per-repo)

Add section markers for selective agent loading:
```markdown
<!-- SQUAD:section:identity -->
<!-- SQUAD:section:architecture -->
<!-- SQUAD:section:data-model -->
<!-- SQUAD:section:api-surface -->
<!-- SQUAD:section:infra -->
<!-- SQUAD:section:ci -->
<!-- SQUAD:section:file-inventory -->
```

## Step 6 — Generate Per-Repo DEEP-CONTEXT.md

For EACH repo, generate `[repo]/DEEP-CONTEXT.md` using the structured template below.
...
### DEEP-CONTEXT.md Template (300-line limit)
...
[Entry points, module structure, key abstractions — derived from Step 2c source scan]

## Data Model
[Database tables/collections, schemas, key types/interfaces — from *.sql, *.proto, *.graphql, migrations/]

## API Surface
[REST endpoints, gRPC services, GraphQL queries/mutations — from openapi.yaml, *.proto, route files]

## Integration Points
[External services, message queues, shared libraries — from package.json, imports, Dockerfiles]

## Deployment Model
[Docker services, Kubernetes resources, CI/CD pipeline — from Dockerfile, docker-compose, .github/workflows/]

## Decision Log
[Extracted from README, CHANGELOG, ADRs — WHY key decisions were made]

## Key Files
[Top 10 most important files with 1-line descriptions — from KG god nodes + highest coupling]

## Test Coverage Summary
[Test framework, test count, coverage gaps — from KG untested files list]
```
